...
public:
    ...
    void RenderWallQuad( FaceInfo &curFace, int nLeftClip, int nRghtClip ) {
        ...
        // NOTE - this line drawing is not clipped! Only use for debug purposes
...
//        DrawLine( left_lower + olc::vf2d(  0, -1 ), rght_lower + olc::vf2d(  0, -1 ), olc::BLUE  );    // floor side
    ...
    // returns true if this occlusionrec should be (partially) rendered. If so, nClipLeft and nClipRght denote
    // clipping values
    ...
    bool InsertOccList( OccListType &lst, OcclusionRec &rec, int &nClipLeft, int &nClipRght ) {
        ...
        // rec must be inserted if A. it overlaps with left element, or B. it overlaps with right element (on left side),
        // or C. it doesn't overlap, but it's after the left and before the right element
        bool bFound = (
        ...
        if (bFound) {
            ...
            // 2b. if location is not found, return false
            // ------------------------------------------
            nClipLeft = -1;
            nClipRght = -1;
        }
